...
namespace BlueDataWeb.Controllers
{
    public class GiaiPhapController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            NewCategory newcategory = db.NewCategories.AsNoTracking().Where(m => m.KeyName == "E_GiaiPhap" && m.AppID == this.AppID).FirstOrDefault();
           ...
           if (newcategory != null)
            ...
            var lstAllSanPham = db.News.AsNoTracking().Where(x => x.NewCategoriesID == newcategory.NewCategoriesID && x.AppID == this.AppID &&
                (x.IsDelete == null || x.IsDelete.Value == false)).OrderBy(x => x.OrderBY).ToList();
            ViewBag.lstAllSanPham = lstAllSanPham;



            return View("GiaiPhap_Index", lstnew);
        }
