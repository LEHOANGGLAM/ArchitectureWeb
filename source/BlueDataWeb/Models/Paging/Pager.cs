...
namespace BlueDataWeb.Paging
{
    public class Pager : IHtmlString
    {
        ...
        public Pager(HtmlHelper htmlHelper, int pageSize, int currentPage, int totalItemCount, RouteValueDictionary valuesDictionary)
        {
        ...
        //public Pager(int pageSize, int currentPage, int totalItemCount, RouteValueDictionary valuesDictionary, AjaxOptions ajaxOptions)
        //{
        //    this.pageSize = pageSize;
        //    this.currentPage = currentPage;
        //    this.totalItemCount = totalItemCount;
        ...
        public virtual PaginationModel BuildPaginationModel(Func<int, string> generateUrl)
        {
            ...
            // Previous
            model.PaginationLinks.Add(currentPage > 1 ? new PaginationLink { Active = true, DisplayText = "Trước", PageIndex = currentPage - 1, Url = generateUrl(currentPage - 1) } : new PaginationLink { Active = false, DisplayText = "Trước" });
            ...
            for (var i = start; i <= end; i++)
            {
                ...
                {
                    // loi.nguyen thêm Url = generateUrl(i) : để gộp chung 2 action khi xử lý paging
            ...
            // Next
            model.PaginationLinks.Add(currentPage < pageCount ? new PaginationLink { Active = true, PageIndex = currentPage + 1, DisplayText = "Sau", Url = generateUrl(currentPage + 1) } : new PaginationLink { Active = false, DisplayText = "Sau" });
        ...
        protected virtual string GeneratePageUrl(int pageNumber)
        {
            ...
            //loinguyen : sửa code để gộp action
           ...
           // }
            ...
            // Customer lại truyền thêm param
            pageLinkValueDictionary = new RouteValueDictionary(linkWithoutPageValuesDictionary) { { this.pagerOptions.PageRouteValueKey, pageNumber } };
    ...
    public class Pager<TModel> : Pager
    {
        ...
        public Pager(HtmlHelper<TModel> htmlHelper, int pageSize, int currentPage, int totalItemCount, RouteValueDictionary valuesDictionary)
            : base(htmlHelper, pageSize, currentPage, totalItemCount, valuesDictionary)
        {
            this.htmlHelper = htmlHelper;
        }
