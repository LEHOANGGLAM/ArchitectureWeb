...
namespace BlueDataWeb.Areas.Admin.Controllers
{
    ...
    public class ChucNangController : BaseController
    {
        ...
        {
            var cate = db.NewCategories.Where(x => x.KeyName == "E_ChucNang" && x.AppID == this.AppID).FirstOrDefault();

            var data = new List<News>();
            if (cate != null)
            {
                data = db.News.Where(x => x.NewCategoriesID.Value == cate.NewCategoriesID
                  &&  (x.IsDelete.Value == false || x.IsDelete == null)).OrderBy(m => m.OrderBY).ToList();
            }
